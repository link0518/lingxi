...
- 2026-02-01
  - 新增“用户消失后 AI 主动关心”后台任务：仅对用户最新会话生效，按 12/24/48 小时三档触发（最多 3 次），用户一发言即重置；新增表 `idle_nudges` 用于幂等与次数控制。
  - 后端新增 LLM 设置持久化与加密：新增表 `llm_settings` 保存用户在“API 设置”页填写的 baseUrl/modelChat/modelLight，API Key 使用 `LLM_SETTINGS_KEY`（AES-256-GCM）加密存储；后台主动消息使用“轻量任务模型”生成。
  - 交互延迟优化：`/api/messages/append` 在 assistant 落库后回传最新好感度/阶段摘要，前端立即更新阶段显示与发送按钮状态，减少“AI 已回复但仍显示忙碌/阶段未更新”的延迟。
  - 前端缓存优化：`apiGetMe` / `apiGetCharacter` 增加 localStorage TTL 缓存（10 分钟），减少切换菜单时重复请求头像。
  - 部署脚本调整：VPS 一键部署/更新脚本改为后端通过 `.env` + `dotenv-cli` 加载环境变量；新增 `.env.example` 说明 `IDLE_NUDGE_*` 与 `LLM_SETTINGS_KEY`。
  - 影响文件：`server/index.js`、`server/db.js`、`src/lib/api.ts`、`src/views/ChatPane.tsx`、`scripts/deploy.sh`、`scripts/update.sh`、`.env.example`。

- 2026-01-31
  - 前端 `apiStreamChat` 注入角色卡设定到 system prompt，确保角色身份/背景/说话习惯等生效。
